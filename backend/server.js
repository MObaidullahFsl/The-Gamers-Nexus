...
app.get("/api/games", async (req, res) => {
  try {
    ...
    const result = pool
      .request()
      .query("SELECT * FROM Games", (err, results) => {
        if (err) throw err;

        if (results.length > 0) console.log(results);
        else console.error("Nothing matched Query!");
      });
    res.json(result.recordset);
  } catch (error) {
...
app.post("/api/signup", async (req, res) => {
  try {
    ...
    if (existingUser.recordset.length > 0) {
      return res
        .status(400)
        .json({ result: "fail", message: "Username already exists" });
    }
    ...
    const result = await pool
      ...
      .query(
        `INSERT INTO Users (username, password) OUTPUT INSERTED.id, INSERTED.username VALUES (@username, @password)`
      );

    if (!result.recordset || result.recordset.length === 0) {
      return res
        .status(400)
        .json({ result: "fail", message: "Registration failed" });
    }
    ...
    res
      .status(201)
      .json({ result: "success", message: "User created!", user: newUser });
  } catch (error) {
    console.error("Signup error:", error);
    res
      .status(500)
      .json({ result: "error", message: error.message, err: error.stack });
  }
});
...
app.post("/api/signin", async (req, res) => {
  try {
    ...
    if (!result.recordset || result.recordset.length === 0) {
      return res
        .status(401)
        .json({ result: "fail", message: "No user with this username found" });
    }
    ...
    if (!match) {
      return res
        .status(401)
        .json({ result: "fail", message: "Invalid password" });
    }
...
app.get("/api/library", async (req, res) => {
  try {
    ...
    const result = await pool
      .request()
      .input("userId", sql.Int, req.session.user.id).query(`
    ...
    res
      .status(500)
      .json({ error: `Failed to fetch user library ${req.session.user}` });
  }
});
  ...
  try {
    ...
    const result = await pool
      ...
      .query(`
        ...
        JOIN Games g ON g.id = Purchases.game_id
...
app.post("/api/friends/add", async (req, res) => {
  try {
    ...
    const existingFriendship = await pool
      ...
      .input("friendId", sql.VarChar, friendId).query(`
    ...
    if (existingFriendship.recordset.length > 0) {
      return res
        .status(400)
        .json({ result: "fail", message: "Friendship already exists." });
    }
    ...
    await pool
      ...
      .input("friendId", sql.VarChar, userId).query(`
        INSERT INTO Friends (user_id, friend_id) VALUES (@userId, @friendId);
      `);

    res
      .status(201)
      .json({ result: "success", message: "Friend added successfully!" });
  } catch (error) {
...
app.get("/api/users/:userId/common-games/:friendId", async (req, res) => {
  try {
    ...
    const friendshipCheck = await pool
      .request()
...
app.get("/api/games/publisher/:publisherId", async (req, res) => {
  try {
    ...
    const result = await pool
      .request()
      .input("publisherId", sql.VarChar, publisherId).query(`
...
app.get("/api/games/:gameId", async (req, res) => {
  try {
    ...
    const result = await pool.request().input("gameId", sql.VarChar, gameId)
      .query(`
      select * 
      from Games
